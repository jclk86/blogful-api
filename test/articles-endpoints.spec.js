...
describe.only("Articles Endpoints", function() {
  ...
  before('make knex instance', () => {
    db = knex({
      client: 'pg',
      connection: `postgresql://postgres:B3Th3B3st@localhost/blogful-test`,
    })
    app.set('db', db) // you need this b/c test skips server.js. The app here is express(), which, from the app.js file, expects req.app.get('db'). 
  })
    ...
    const testArticles = [
     ...
     beforeEach('insert articles', () => {
      ...
      it('GET /article/:article_id responds with 200 and the specified article', () => {
        const articleId= 2;
        const expectedArticle = testAticles[articleId - 1]
        return supertest(app)
